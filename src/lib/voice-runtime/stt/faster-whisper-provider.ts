...
const FASTER_WHISPER_PYTHON_HELPER = [
  ...
  "parser.add_argument('--audio-ref',dest='audio_ref',required=True)",
  "parser.add_argument('--model-path',dest='model_path',default='')",
  "parser.add_argument('--model-name',dest='model_name',default='')",
  "parser.add_argument('--beam-size',dest='beam_size',type=int,required=True)",
  "parser.add_argument('--language')",
  "parser.add_argument('--vad-enabled',dest='vad_enabled',choices=['true','false'],default='false')",
  "args=parser.parse_args()",
  "model_target=args.model_name.strip() or args.model_path.strip()",
  "if not model_target: raise SystemExit('faster-whisper requires --model-name or --model-path')",
  "started=time.perf_counter()",
  "model=WhisperModel(model_target,device='cpu',compute_type='int8')",
  "segments,info=model.transcribe(args.audio_ref,beam_size=args.beam_size,language=args.language or None,vad_filter=args.vad_enabled=='true')",
