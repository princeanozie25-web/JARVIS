...
function proposalAuditPreview() {
  return buildApprovalAuditPreviewContract({
    preview_id_hash: "hash:compensation-validation-proposal-preview",
    proposal: proposal(),
...
function inboxItem() {
  return buildApprovalProposalInboxItem({
    inbox_item_id: "inbox:compensation-validation-item",
    proposal_id: "proposal:compensation-validation-item",
    proposal: proposal(),
...
function reviewDecision() {
  return buildApprovalReviewDecisionMetadata({
    review_decision_id: "review:compensation-validation-decision",
    inbox_item: inboxItem(),
    decision_request: "approve_requested",
    channel: "typed_confirmation",
    actor_ref_hash: "hash:local-user",
    reason_ref_hash: "hash:compensation-validation-reason",
...
function reviewSession() {
  return buildApprovalReviewSessionSnapshot({
    review_session_id: "review-session:compensation-validation-session",
    inbox_item: inboxItem(),
    audit_preview: proposalAuditPreview(),
...
function decisionRecord() {
  return buildApprovalDecisionRecordMetadata({
    decision_record_id: "decision-record:compensation-validation-record",
    review_session: reviewSession(),
    outcome: "approved_recorded",
    channel: "typed_confirmation",
    actor_ref_hash: "hash:local-user",
    reason_ref_hash: "hash:compensation-validation-record-reason",
    reason_kind: "user_intent_metadata",
  });
}
...
function executionPlan() {
  return buildApprovalExecutionPlanMetadata({
    execution_plan_id: "execution-plan:phase-18g2",
    decision_record: decisionRecord(),
    target_class: "obsidian_note",
    target_ref_hash: "hash:target-note",
    risk_class: "medium",
    step_id: "step:phase-18g2-note-create",
  });
}

function verificationMetadata() {
  return buildApprovalExecutionVerificationMetadata({
    verification_id: "verification:phase-18g2",
    execution_plan: executionPlan(),
    evidence_id: "verification-evidence:phase-18g2",
    redacted_reference: "redacted:phase-18g2-verification-evidence",
    hash_reference: "hash:phase-18g2-verification-evidence",
...
function compensationMetadata() {
  return buildApprovalExecutionCompensationMetadata({
    compensation_id: "compensation:phase-18g2",
    verification_metadata: verificationMetadata(),
    hint_id: "compensation-hint:phase-18g2",
    evidence_id: "compensation-evidence:phase-18g2",
    redacted_reference: "redacted:phase-18g2-compensation",
    hash_reference: "hash:phase-18g2-compensation",
  });
}

function resultFor(
  input: unknown,
  guardId: (typeof APPROVAL_EXECUTION_COMPENSATION_VALIDATION_GUARD_IDS)[number],
) {
  const result = validateApprovalExecutionCompensationPolicyMetadata(
    input,
...
describe("Phase 18G.2 execution compensation validation guard", () => {
  it("defines the metadata-only compensation validation guard contract", () => {
    expect(
      DEFAULT_APPROVAL_EXECUTION_COMPENSATION_VALIDATION_POLICY_MATRIX,
    ).toMatchObject({
      contract_version: "18G.2",
  ...
  it("declares all required guards as inert metadata", () => {
    const guards = [
      ...DEFAULT_APPROVAL_EXECUTION_COMPENSATION_VALIDATION_POLICY_MATRIX.compensation_guards,
      ...DEFAULT_APPROVAL_EXECUTION_COMPENSATION_VALIDATION_POLICY_MATRIX.strategy_guards,
      ...DEFAULT_APPROVAL_EXECUTION_COMPENSATION_VALIDATION_POLICY_MATRIX.hint_guards,
      ...DEFAULT_APPROVAL_EXECUTION_COMPENSATION_VALIDATION_POLICY_MATRIX.evidence_guards,
      ...DEFAULT_APPROVAL_EXECUTION_COMPENSATION_VALIDATION_POLICY_MATRIX.disabled_authority_guards,
    ];
    const declaredGuardIds = guards.map((guard) => guard.guard_id);

    expect(declaredGuardIds).toHaveLength(
      APPROVAL_EXECUTION_COMPENSATION_VALIDATION_GUARD_IDS.length,
    );
  ...
  it("passes valid metadata-only compensation records", () => {
    const results = validateApprovalExecutionCompensationPolicyMetadata(
    ...
    for (const result of results) {
      expect(
        ApprovalExecutionCompensationValidationGuardResultSchema.safeParse(
          result,
        ).success,
      ).toBe(true);
  ...
  it("fails operational statuses and unknown compensation strategies", () => {
    expect(
      resultFor(
        {
          ...compensationMetadata(),
      ...
      passed: false,
      reason_code: "operational_compensation_status",
    });
    ...
    expect(
      resultFor(
        {
          ...compensationMetadata(),
          strategy_metadata: {
            ...compensationMetadata().strategy_metadata,
            strategy: "real_inverse_operation",
          },
        },
      ...
      passed: false,
      reason_code: "unknown_compensation_strategy",
    });
  });

  it("fails enabled compensation, rollback, restore, execution, and dispatch flags", () => {
    const flagToGuard = {
  ...
  it("fails raw hint and evidence fields", () => {
    expect(
      resultFor(
        {
          ...compensationMetadata(),
          hint_metadata: [
            {
              ...compensationMetadata().hint_metadata[0],
      ...
      resultFor(
        {
          ...
          evidence_metadata: [
            {
              ...compensationMetadata().evidence_metadata[0],
  ...
  it("fails raw payloads, prompts, model output, project, memory, and device content", () => {
    const hintCases = [
      ["prompt", "hint_no_prompts", "prompt_present"],
      ["model_output", "hint_no_model_output", "model_output_present"],
      [
        "project_contents",
    ...
    for (const [field, guardId, reasonCode] of hintCases) {
      expect(
        resultFor(
          {
            ...compensationMetadata(),
  ...
  it("exposes no operational public exports for compensation validation", () => {
