...
export const APPROVAL_EXECUTION_VERIFICATION_VALIDATION_CONTRACT_VERSION =
  "18F.2" as const;

export const APPROVAL_EXECUTION_VERIFICATION_VALIDATION_GUARD_IDS = [
...
export const APPROVAL_EXECUTION_VERIFICATION_VALIDATION_GUARD_SEVERITIES = [
  "info",
  "warning",
  "error",
] as const;

export const APPROVAL_EXECUTION_VERIFICATION_VALIDATION_REASON_CODES = [
  "passed",
  "unknown_verification_status",
  "operational_verification_status",
  "unknown_verification_method",
  "verification_not_replay_safe",
  "verification_not_redaction_safe",
  "raw_payload_present",
  ...
  "invalid_verification_metadata_shape",
] as const;

export type ApprovalExecutionVerificationValidationGuardId =
  (typeof APPROVAL_EXECUTION_VERIFICATION_VALIDATION_GUARD_IDS)[number];
export type ApprovalExecutionVerificationValidationGuardSeverity =
  (typeof APPROVAL_EXECUTION_VERIFICATION_VALIDATION_GUARD_SEVERITIES)[number];
export type ApprovalExecutionVerificationValidationReasonCode =
  (typeof APPROVAL_EXECUTION_VERIFICATION_VALIDATION_REASON_CODES)[number];

export const ApprovalExecutionVerificationValidationGuardIdSchema = z.enum(
  APPROVAL_EXECUTION_VERIFICATION_VALIDATION_GUARD_IDS,
);
export const ApprovalExecutionVerificationValidationGuardSeveritySchema =
  z.enum(APPROVAL_EXECUTION_VERIFICATION_VALIDATION_GUARD_SEVERITIES);
export const ApprovalExecutionVerificationValidationReasonCodeSchema = z.enum(
  APPROVAL_EXECUTION_VERIFICATION_VALIDATION_REASON_CODES,
);

export const ApprovalExecutionVerificationValidationGuardDeclarationSchema =
  z.strictObject({
    guard_id: ApprovalExecutionVerificationValidationGuardIdSchema,
    applies_to: z.enum([
      "execution_verification",
      "verification_evidence_metadata",
      "verification_method_metadata",
      "disabled_authority_flags",
    ]),
    severity: ApprovalExecutionVerificationValidationGuardSeveritySchema,
    failure_reason_code:
...
export const ApprovalExecutionVerificationValidationGuardResultSchema =
  z.strictObject({
    guard_id: ApprovalExecutionVerificationValidationGuardIdSchema,
    passed: z.boolean(),
    severity: ApprovalExecutionVerificationValidationGuardSeveritySchema,
    reason_code: ApprovalExecutionVerificationValidationReasonCodeSchema,
    redaction_status: ApprovalRedactionStatusSchema,
    replay_safe: z.literal(true),
    redaction_safe: z.literal(true),
    checked_at_source: z.literal(
      "approval_execution_verification_validation_matrix",
    ),
    metadata_only: z.literal(true),
    audit_preview_safe: z.literal(true),
...
export const ApprovalExecutionVerificationValidationPolicyMatrixSchema =
  z.strictObject({
    contract_version: z.literal(
      APPROVAL_EXECUTION_VERIFICATION_VALIDATION_CONTRACT_VERSION,
    ),
    matrix_id: z.literal("approval_execution_verification_validation_matrix"),
    phase: z.literal(18),
    slice: z.literal("18F.2"),
    metadata_only: z.literal(true),
...
export type ApprovalExecutionVerificationValidationGuardDeclaration = z.infer<
  typeof ApprovalExecutionVerificationValidationGuardDeclarationSchema
>;
export type ApprovalExecutionVerificationValidationGuardResult = z.infer<
  typeof ApprovalExecutionVerificationValidationGuardResultSchema
>;
export type ApprovalExecutionVerificationValidationPolicyMatrix = z.infer<
  typeof ApprovalExecutionVerificationValidationPolicyMatrixSchema
>;

const GUARD_REASON_BY_ID = {
  known_inert_verification_status_only: "unknown_verification_status",
  operational_status_rejected: "operational_verification_status",
  known_metadata_only_verification_method_only: "unknown_verification_method",
  verification_replay_safe: "verification_not_replay_safe",
  verification_redaction_safe: "verification_not_redaction_safe",
...
const GUARD_APPLIES_TO = {
  known_inert_verification_status_only: "execution_verification",
  operational_status_rejected: "execution_verification",
  known_metadata_only_verification_method_only: "verification_method_metadata",
  verification_replay_safe: "execution_verification",
  verification_redaction_safe: "execution_verification",
  raw_payloads_forbidden: "execution_verification",
  secrets_forbidden: "execution_verification",
  evidence_metadata_only: "verification_evidence_metadata",
  evidence_redacted_reference_only: "verification_evidence_metadata",
  evidence_hash_reference_only: "verification_evidence_metadata",
  evidence_no_raw_state: "verification_evidence_metadata",
  evidence_no_raw_device_payload: "verification_evidence_metadata",
  evidence_no_raw_tool_output: "verification_evidence_metadata",
  evidence_no_raw_project_content: "verification_evidence_metadata",
  evidence_no_raw_memory_content: "verification_evidence_metadata",
  evidence_no_model_output: "verification_evidence_metadata",
  evidence_no_prompts: "verification_evidence_metadata",
  ...
  ApprovalExecutionVerificationValidationGuardId,
  ApprovalExecutionVerificationValidationGuardDeclaration["applies_to"]
>;

function guardDeclaration(
  guard_id: ApprovalExecutionVerificationValidationGuardId,
): ApprovalExecutionVerificationValidationGuardDeclaration {
  return ApprovalExecutionVerificationValidationGuardDeclarationSchema.parse({
    guard_id,
...
export const DEFAULT_APPROVAL_EXECUTION_VERIFICATION_VALIDATION_POLICY_MATRIX =
  ApprovalExecutionVerificationValidationPolicyMatrixSchema.parse({
    contract_version:
      APPROVAL_EXECUTION_VERIFICATION_VALIDATION_CONTRACT_VERSION,
    ...
    method_guards: (
      ["known_metadata_only_verification_method_only"] as const
    ).map(guardDeclaration),
...
function guardResult(input: {
  readonly guard_id: ApprovalExecutionVerificationValidationGuardId;
  readonly passed: boolean;
  readonly reason_code?: ApprovalExecutionVerificationValidationReasonCode;
}): ApprovalExecutionVerificationValidationGuardResult {
  return ApprovalExecutionVerificationValidationGuardResultSchema.parse({
...
function methodValue(input: unknown): unknown {
  const methodMetadata = recordField(input, "method_metadata");
  if (
    methodMetadata &&
    typeof methodMetadata === "object" &&
    "method" in methodMetadata
  ) {
    return (methodMetadata as { readonly method?: unknown }).method;
  }

  return undefined;
}

function evidenceItems(input: unknown): readonly Record<string, unknown>[] {
  const evidence = recordField(input, "evidence_metadata");
...
function everyEvidenceField(
  ...
  return (
    evidence.length > 0 && evidence.every((item) => predicate(item[field]))
  );
}
...
function evidenceHasForbiddenKey(
  input: unknown,
  keys: ReadonlySet<string>,
): boolean {
  return evidenceItems(input).some((item) => hasForbiddenKey(item, keys));
}
...
function evidenceMetadataOnly(input: unknown): boolean {
  return everyEvidenceField(
    input,
    "redaction_status",
    (value) =>
      !!value &&
...
export function validateApprovalExecutionVerificationPolicyMetadata(
  input: unknown,
): readonly ApprovalExecutionVerificationValidationGuardResult[] {
  ...
  return [
    guardResult({
      guard_id: "known_inert_verification_status_only",
      passed:
        ApprovalExecutionVerificationStatusSchema.safeParse(status).success &&
        !ApprovalExecutionVerificationForbiddenStatusSchema.safeParse(status)
          .success,
    }),
    guardResult({
    ...
    guardResult({
      guard_id: "known_metadata_only_verification_method_only",
      passed:
        ApprovalExecutionVerificationMethodSchema.safeParse(method).success,
    }),
    guardResult({
      guard_id: "verification_replay_safe",
      passed: hasReplaySafeMetadata(input),
    }),
    guardResult({
      guard_id: "verification_redaction_safe",
    ...
    guardResult({
      ...
      passed: evidenceMetadataOnly(input),
    }),
    guardResult({
      ...
      passed:
        ...
        ) && !evidenceHasForbiddenKey(input, new Set(["raw_reference"])),
    }),
    guardResult({
      guard_id: "evidence_hash_reference_only",
      passed: everyEvidenceField(
        input,
        "hash_reference",
    ...
    guardResult({
      ...
      passed: !evidenceHasForbiddenKey(input, new Set(["raw_state", "state"])),
    }),
    guardResult({
      guard_id: "evidence_no_raw_device_payload",
      passed: !evidenceHasForbiddenKey(input, new Set(["device_payload"])),
    }),
    guardResult({
      guard_id: "evidence_no_raw_tool_output",
      passed: !evidenceHasForbiddenKey(
        input,
        new Set(["tool_output", "tool_outputs"]),
      ),
    }),
    guardResult({
      guard_id: "evidence_no_raw_project_content",
      passed: !evidenceHasForbiddenKey(input, new Set(["project_contents"])),
    }),
    guardResult({
      guard_id: "evidence_no_raw_memory_content",
      passed: !evidenceHasForbiddenKey(input, new Set(["memory_contents"])),
    }),
    guardResult({
      guard_id: "evidence_no_model_output",
      passed: !evidenceHasForbiddenKey(
        input,
        new Set(["model_output", "model_outputs"]),
      ),
    }),
    guardResult({
      guard_id: "evidence_no_prompts",
      passed: !evidenceHasForbiddenKey(input, new Set(["prompt", "prompts"])),
    }),
    guardResult({
