...
- The cube above is a Blueprint created using the **Shape_Cube** mesh and its collision has been set to **OverlapOnlyPawn** so it acts as a trigger. Also enable **Generate Overlap Events**
...
1. In the **Shape_Cube** Blueprint, under **My Blueprint** click the button on the Variables category.

2. In the **Details** panel, under **Variable Type** select the type of Blueprint you wish to access. Hover over the **Blueprint_Effects_Sparks** Variable Type and select **Reference** from the list. Here we are stating that we want to access a **Blueprint_Effect_Sparks** Blueprint.
...
4. With the **Shape_Cube** Blueprint selected in the level, in the Level Editor under **Details** you should see the exposed variable created in the previous step.
...
Here we are stating which **Blueprint_Effect_Sparks** Blueprint Actor we want to affect that is placed in our level; this is considered the Instance Actor. If we had multiple sparks in our level and we only wanted to turn off one of them, we could choose which instance of the Blueprint we want to affect here by setting it as the **Target Blueprint**.

5. Instead of using the drop-down, you can click the eyedropper icon, then click on the object you want to reference that is placed in your level. You can only set the **Target Blueprint** to target the specified Variable Type (in our case **Blueprint_Effect_Sparks**).

6. In the **Shape_Cube** Blueprint, while holding down the **Ctrl** key, drag the variable into the graph.
...
8. Off the **StartFire_Event**, add a **SpawnActorFromClass** node (class set to **Blueprint_Effect_Fire**) and for the **Transform**, get the **SM_Bush** Transform.

9. Off the **Return Value** of the Spawn Actor node, add the **Assign On Destroyed** node.

10. Off the **OnDestroyed_Event**, add a **Destroy Actor** node with target being **SM_Bush**.
...
9. Open the **Blueprint_Effect_Fire** Blueprint, then click the **Blueprint Props** button from the toolbar.

10. In the **Details** panel under **Interfaces**, click the **Add** button then select your Interface (**CubeInterface_C** in our example).
...
13. Open the **Blueprint_CeilingLight** Blueprint, click **Blueprint Props** then add the Interface from the **Details** panel as before.
...
15. Repeat the process of adding **Blueprint Props** to the **Blueprint_Effect_Sparks** Blueprint, then add the **CubeInterface_C**. Here we are moving the spark effect up when entering the cube, then down when entering the cube a second time.

16. Repeat the process of adding **Blueprint Props** to the **MyCharacter** Blueprint, then add the **CubeInterface_C**. Here we increase the character's movement speed each time they enter the cube by 100.
...
Below we have the **Blueprint_Effect_Fire** Blueprint which will represent the player character's life force. This Blueprint will check what the player's health is and once it is 0, will dissolve and disappear.
...
2. Name the Interface, **BP_Interface** (or some other name) then **Double-click** on it to open it up and click the **Add Function** button.
...
6. In the **Details** panel under **Interfaces**, click the **Add** button then select your Interface (**BP_Interface_C** in our example).
...
11. Open the Blueprint you want to pass the variables to (**Blueprint_Effect_Fire**), and click the **Blueprint Props** button, then **Add** the interface via the **Details** panel.
...
13. Off the Binded Event **TakeDamage_Event**, add the **GetHealth** Interface Message. Be sure to implement the **Interface Message** and not the **Call Function**.
