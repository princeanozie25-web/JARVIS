...
- Job Scout has entered workflow implementation with source contracts, deterministic fixture-input ranking, fit scoring, missing-skill analysis, digest generation, and Suggestion Inbox-ready digest payloads. Morning Brief can include supplied Job Scout digest metadata as an optional section. Scraping, Playwright/browser automation, external job-board APIs, application automation, submissions, auto-apply, and real Suggestion Inbox writes remain future slices; the current Job Scout path is suggestion-only and does not call models or the network.
