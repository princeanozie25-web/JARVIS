...
- LLM Wiki has preview/draft foundations, and Knowledge Compounding is complete through the human approval boundary: hub candidate selection, candidate ranking, vault-sourced draft generation, source attribution, approval-gated write planning, and re-index planning exist. Actual vault writes, automatic wiki creation, scheduler-driven mutation, autonomous knowledge growth, and gateway execution remain future slices.
...
- Google adapter work is T0 read closeout-complete for Gmail, Calendar, and Drive, with a shared Google Account Runtime for OAuth state, token validity metadata, scope inventory, and adapter readiness. Authenticated read-only metadata retrieval exists for Gmail recent/unread/single-message metadata, Calendar upcoming/today/single-event metadata, and Drive recent/search/single-file metadata. Gmail send/draft/mutation, Calendar event creation/update/delete/RSVP, Drive file creation/update/delete/permission changes/downloads, background sync, and live MCP wiring remain disabled.
- Morning Brief is complete as a governed suggestion-only workflow: it consumes supplied Gmail, Calendar, and optional Drive metadata inputs, composes deterministic previews, produces Suggestion Inbox-ready digest payloads, and declares disabled-by-default scheduler metadata for daily 08:00 local delivery. It does not run a background daemon, call live Google adapters directly, write to the real Suggestion Inbox, auto-send, auto-execute, mutate, or approve anything; any future action from a brief must pass through the existing approval lifecycle.
- Job Scout workflow is complete through the human approval boundary: source contracts, supplied feed ingestion, feed validation/summary, normalization into job postings, deterministic ranking, fit scoring, missing-skill analysis, digest generation, Suggestion Inbox-ready digest payloads, optional Morning Brief digest metadata integration, application tracking, cover-letter draft planning, and application workflow planning exist. Scraping, Playwright/browser automation, external job-board APIs, LLM-generated cover letters, application submission, auto-apply, auto-send, and real Suggestion Inbox writes remain future slices; the current Job Scout path is suggestion-only and does not call models or the network.
- Telegram inbound transport is complete as a transport-only inlet: direct 1:1 text message contracts, deterministic normalization, router envelopes, and metadata-only conversation state exist. Telegram API calls, bot tokens, outbound messaging, media handling, voice, images, files, groups, channels, agent execution, approval actions, and mobile workflows remain future slices.
- Council Mode is complete through the advisory reasoning boundary: deterministic council member responses, anonymous cross-review, chairman synthesis, confidence/disagreement summaries, and advisory final answer generation exist. Real provider participation, live model calls, execution decisions, action approvals, autonomous reasoning loops, mutations, and authority escalation remain future/forbidden outside a separately governed opening.
...
Still future/not started in Phase 21: social media extraction, Graphify overlay, and pipeline visualization style upgrade. MacBook-only Phases 22-26 remain future planned capability openings.
