...
- Morning Brief has a real-input contract plus deterministic composer and preview generator for supplied Gmail, Calendar, and optional Drive metadata. It is not scheduled, does not write to Suggestion Inbox, does not deliver automatically, and does not call live Google adapters directly in the composer/preview slice; Google data may only enter through governed T0 read adapters, not through background sync.
