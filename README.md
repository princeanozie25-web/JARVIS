...
- Job Scout foundation is complete: source contracts, supplied feed ingestion, feed validation/summary, normalization into job postings, deterministic ranking, fit scoring, missing-skill analysis, digest generation, Suggestion Inbox-ready digest payloads, and optional Morning Brief digest metadata integration exist. Scraping, Playwright/browser automation, external job-board APIs, cover-letter generation, application automation, submissions, auto-apply, and real Suggestion Inbox writes remain future slices; the current Job Scout path is suggestion-only and does not call models or the network.
