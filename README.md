...
- Google adapter work has opened narrow Gmail and Calendar T0 read integrations: OAuth configuration/connection helpers and authenticated read-only metadata retrieval exist for Gmail recent/unread/single-message metadata and Calendar upcoming/today/single-event metadata. Drive remains readiness-only. Gmail send/draft/mutation, Calendar event creation/update/delete/RSVP, background sync, and live MCP wiring remain disabled.
