...
- Social media extraction is execution-enabled as a user-triggered, injected-runner workflow. JARVIS can classify supported URLs, enforce source policy, plan yt-dlp download, plan ffmpeg adaptive frame extraction, use injected transcription and analysis runners, assemble a multimodal packet, return structured timestamped analysis metadata, and require temp workspace cleanup. It does not watch URLs, bulk download, persist raw media, store raw transcript/frame/audio/video data in telemetry, call providers directly, or bypass cloud cost/user-trigger gates.
...
Phase 21 closeout is now unblocked across 21A-21K. Still future after Phase 21: UI refurbishment pass, cinematic redesign, recruiter screenshots, Three.js enhancements, real provider benchmarking, autonomous council triggering, richer Telegram media/mobile workflows, and MacBook-only Phases 22-26.
