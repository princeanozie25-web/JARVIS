...
- Agent Runtime has a shared contract, registry, planner, dry-run executor, output factory, unified preview-suite summary, closeout report, and preview agents for Life Coach, Build Monitor, Research Agent, CV Maintenance, Application Tracker, Deadline Agent, Cost Monitor, and Health Agent. A shared Suggestion Inbox delivery bridge now exists for future agent realization, defaulting to dry-run/no-write unless an explicit local/injected adapter is supplied; real scheduling and live reads remain future work.
