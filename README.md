...
- Google adapter work has opened narrow Gmail and Calendar T0 read integrations plus a shared Google Account Runtime for OAuth state, token validity metadata, scope inventory, and adapter readiness. Authenticated read-only metadata retrieval exists for Gmail recent/unread/single-message metadata and Calendar upcoming/today/single-event metadata. Drive remains readiness-only/future. Gmail send/draft/mutation, Calendar event creation/update/delete/RSVP, background sync, and live MCP wiring remain disabled.
- Morning Brief has a metadata-only preview foundation. It does not schedule delivery, send notifications, or read live Google data directly; Gmail and Calendar data may only enter through governed T0 read adapters, not through background sync.
