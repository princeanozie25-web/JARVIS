...
E-007 | WorkingCockpit TTS provider chip is synthetic | WorkingCockpit.tsx:94 — VOICE_STACK = buildSystemVoiceStackRuntimeState() called once at module scope, data-voice-tts-provider + TTS StatusPill both display a fixed value, not live provider selection | RESOLUTION DEFERRED TO 23G: if the WorkingCockpit surface is otherwise live-fed, wire to live provider state; if the surface is uniformly synthetic/fixture-driven, the chip is consistent and the entry closes as no-change | CONFIRMED-SYNTHETIC
