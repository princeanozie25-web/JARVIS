...
OPEN. §2.1 closed 2026-09-07 (rustup, user-space, `~/.cargo`). **G1 APPLIED (E-053)**: standalone build proven on loopback with a packaged data dir and the health route; shell crate builds. **G2+G3 APPLIED (E-054)**: `scripts/package/build-mac.ts --install` produces and installs `~/Applications/JARVIS Command Center.app` (186 MB); cold launch → sidecar on 127.0.0.1:3117 → health 200 → window. Four live findings (TCC under ~/Desktop, JIT entitlement, missing turbo route runtime, symlinked native-addon alias) are fixed in the script/overlay and pinned by `tests/phase-25/packaging-g2.test.ts`. Remaining: G4 (install-root seam for the doctor, sidecar supervisor, bootstrap projection) and G5 (voice round-trip from the packaged app, visual check of the window). §2.2 decided: ad-hoc signing, no notarisation. §2.3 decided: `~/Library/Application Support/JARVIS` via `JARVIS_DATA_DIR`.
